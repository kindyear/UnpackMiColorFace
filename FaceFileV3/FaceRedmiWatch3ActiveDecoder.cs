...
namespace UnpackMiColorFace.FaceFileV3
{
    ...
    {
        public FaceWidget GetWidget(int sectionId, List<string> imageNameList, int posX, int posY, int width,
            int height, uint color = 0)
        {
            var percentLevel = -10;
            var numIndex = 0;
            var colorPostFix = "";
            if (color != 0) colorPostFix = $"_color[{color:X6}]";
            ...
            return sectionId switch
            {
                0x04 => new FaceWidgetImageList
                {
                    ...
                    BitmapList = imageNameList.Aggregate("",
                        (final, next) => final + (final.Length > 0 ? "|" : "") + $"({percentLevel += 10}):{next}"),
                    X = posX,
                    Y = posY,
                    Width = width,
                    Height = height,
                ...
                0x10 => new FaceWidgetImageList
                {
                    ...
                    BitmapList = imageNameList.Aggregate("",
                        (final, next) => final + (final.Length > 0 ? "|" : "") + $"({numIndex += 1}):{next}"),
                    X = posX,
                    Y = posY,
                    Width = width,
                    Height = height,
                ...
                0x42 => new FaceWidgetImageList
                {
                    ...
                    BitmapList = imageNameList.Aggregate("",
                        (final, next) => final + (final.Length > 0 ? "|" : "") + $"({numIndex++}):{next}"),
                    X = posX,
                    Y = posY,
                    Width = width,
                    Height = height,
